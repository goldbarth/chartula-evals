...
| 2026-09-04 | `docs/output-format.md` rule 8 says "Scope, outcome and action are omitted when they do not apply, per C2 to C4". C3 has no not-applicable case: only C4 defines `n/a`, and C2 calls its not-applicable case a pass in as many words. So the format permits an omission the axis it points at never allows. | Repairing the sentence edits the criterion, so it waits for a cycle where the rubric is open. Nothing waits on it in the meantime: the permission had reached Chartula's customer prompt, and that half is fixed there. The turn it was measured on is in [`measurements.md`](measurements.md). |
