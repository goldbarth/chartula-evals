...
- Add inline citation badges to sanitized assistant Markdown answers, with hover and focus previews and numbered sources-list entries. ([#245](https://github.com/goldbarth/ServiceDeskLite/pull/245))
- Make the first actionable `SuggestedNextSteps` item a primary ticket control that uses the existing action handler. ([#237](https://github.com/goldbarth/ServiceDeskLite/pull/237))
- Add a keyboard-operated `CommandPalette` available on every page through Ctrl/Cmd+K, with navigation and ticket search results. ([#236](https://github.com/goldbarth/ServiceDeskLite/pull/236))
- Update ticket status optimistically on the board and detail view, restoring the previous state if the server rejects the change. ([#235](https://github.com/goldbarth/ServiceDeskLite/pull/235))
- Replace ticket status and assignee dialogs with inline `MudMenu` controls. ([#234](https://github.com/goldbarth/ServiceDeskLite/pull/234))
- Add inline citation badges to assistant answers, with hover and keyboard-focus previews. ([#233](https://github.com/goldbarth/ServiceDeskLite/pull/233))
...
- Force `check_grounding` through `tool_choice` after passage retrieval when no grounding check has run. ([#218](https://github.com/goldbarth/ServiceDeskLite/pull/218))
- Centralize palette, spacing, radius, elevation, and semantic chip colors in `Theme/DesignTokens.cs`. ([#213](https://github.com/goldbarth/ServiceDeskLite/pull/213))

### Fixed

- Wrap board card titles in `board-card__title-row` so long titles clamp to two lines. ([#240](https://github.com/goldbarth/ServiceDeskLite/pull/240))
- Fix the scoped `.assistant-chat` styling so the composer send button stays inside its panel. ([#239](https://github.com/goldbarth/ServiceDeskLite/pull/239))
- Fix the `Category` column width and header styling so it does not overlap `Due` in the ticket queue. ([#238](https://github.com/goldbarth/ServiceDeskLite/pull/238))
- Clarify the chat system prompt's confirmation policy so the assistant reports writes only after a successful tool result and does not ask for needless reconfirmation. ([#223](https://github.com/goldbarth/ServiceDeskLite/pull/223))
- Render ticket summary timestamps in `Anthropic:UserTimeZone` rather than UTC. ([#222](https://github.com/goldbarth/ServiceDeskLite/pull/222))
...
- Score grounding with `SemanticGroundingEvaluator` using passage similarity, falling back to `LexicalGroundingEvaluator` when embeddings are unavailable or fail. ([#219](https://github.com/goldbarth/ServiceDeskLite/pull/219))
