...
- Integrate the M9 Frontend Redesign milestone into `main` for v1.9.0. ([#244](https://github.com/goldbarth/ServiceDeskLite/pull/244))
...
- Add a primary ticket action based on the first actionable `SuggestedNextSteps` item. ([#237](https://github.com/goldbarth/ServiceDeskLite/pull/237))
- Add a `CommandPalette` available on every page through Ctrl/Cmd+K. ([#236](https://github.com/goldbarth/ServiceDeskLite/pull/236))
- Update ticket statuses optimistically on the board and detail view, with rollback when a change is rejected. ([#235](https://github.com/goldbarth/ServiceDeskLite/pull/235))
...
- Add the active agent roster to the chat system prompt for ticket assignment. ([#221](https://github.com/goldbarth/ServiceDeskLite/pull/221))
- Require `AgentLoop` to call `check_grounding` through `tool_choice` after retrieval when no check has run. ([#218](https://github.com/goldbarth/ServiceDeskLite/pull/218))
...
- Fix the assistant composer layout so its send button stays inside the panel. ([#239](https://github.com/goldbarth/ServiceDeskLite/pull/239))
- Fix the ticket queue `Category` header so it no longer overlaps `Due`. ([#238](https://github.com/goldbarth/ServiceDeskLite/pull/238))
- Add a chat system-prompt policy that requires successful tool results before claiming a write and avoids unnecessary confirmation. ([#223](https://github.com/goldbarth/ServiceDeskLite/pull/223))
- Render ticket summary timestamps in the timezone configured by `Anthropic:UserTimeZone`. ([#222](https://github.com/goldbarth/ServiceDeskLite/pull/222))
...
- Score grounded answers with `SemanticGroundingEvaluator` against retrieved passages, retaining `LexicalGroundingEvaluator` as a fallback. ([#219](https://github.com/goldbarth/ServiceDeskLite/pull/219))
