sonnet-5-format-out with the published serialisation's front matter prepended to
the customer section. Same entries, byte for byte; only the opening is added.

Built by hand to answer one question for the price of one call: is the missing
front matter the whole of what B2 fails on, or is there more. `title` and
`publishedAt` come from the tag and its date. `tags` is omitted because the fact
base carries no labels to build it from, which the format allows and #98 covers.
`description` is written by hand - nothing in Chartula produces one today, and
that is part of the finding rather than a detail of this file.
...
---
title: Release 0.1.0
description: The first release that generates changelogs end to end, from pull requests to published text.
